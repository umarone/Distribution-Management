...
namespace Accounts.DAL
{
    public class ItemsDAL
    {
        ...
        public List<ItemsEL> GetAllInActiveItems(Int64 IdProject, SqlConnection objConn)
        {
            List<ItemsEL> list = new List<ItemsEL>();
            SqlCommand cmdItems = new SqlCommand("[Setup].[Proc_GetAllInActiveItems]", objConn);
        ...
        {
            List<ItemsEL> list = new List<ItemsEL>();
            SqlCommand cmdProduct = new SqlCommand("[Reports].[Proc_GetProductLedgerByDate]", objConn);
            cmdProduct.Parameters.Add(new SqlParameter("@IdItem", DbType.Int64)).Value = IdItem;
            cmdProduct.Parameters.Add(new SqlParameter("@StartDate", DbType.DateTime)).Value = StartDate;
            cmdProduct.Parameters.Add(new SqlParameter("@EndDate", DbType.DateTime)).Value = EndDate;
            cmdProduct.CommandType = CommandType.StoredProcedure;
            objReader = cmdProduct.ExecuteReader();
            while (objReader.Read())
            {
                ItemsEL oelItem = new ItemsEL();
                oelItem.Description = Validation.GetSafeString(objReader["Description"]);
