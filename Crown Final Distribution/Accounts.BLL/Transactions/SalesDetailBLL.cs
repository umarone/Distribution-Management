...
namespace Accounts.BLL
{
    public class SalesDetailBLL
    {
        ...
        public DataTable GetAllSalesManSaleSummaryDateWise(Int64 IdProject, Int64 BookNo, DateTime dtReport)
        {
            SqlConnection objconn = new SqlConnection(DBHelper.DataConnection);
            try
            {
                objconn.Open();
                return dal.GetAllSalesManSaleSummaryDateWise(IdProject, BookNo, dtReport, objconn);
            }
            catch (Exception ex)
            {
                objconn.Close();
