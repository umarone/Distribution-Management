        ...
        private void grdFindAccount_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == (char)Keys.Enter)
            {
                if (grdFindAccounts.CurrentRow != null)
                {
                    ...
                    oelAccount.Discription = Validation.GetSafeString(grdFindAccounts.Rows[RowIndex].Cells[7].Value);
                    oelAccount.AccountType = Validation.GetSafeString(grdFindAccounts.Rows[RowIndex].Cells[8].Value);
                    this.Close();
                }
            }
            else
