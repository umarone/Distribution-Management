    ...
    public partial class frmDistributionSales : MetroForm
    {
            ...
            if (list.Count > 0)
            {
                ...
                if (objActiveType != null)
                {
                    ...
                    else if (objActiveType.SoftwareType == "Distribution Trading")
                    {
                        ...
                        DgvStockSales.Columns["colItemName"].Width = 220;
                        DgvStockSales.Columns["colpacking"].Width = 70;
                        DgvStockSales.Columns["colCurrentStock"].Width = 70;
                        DgvStockSales.Columns["colQty"].Width = 65;
                        DgvStockSales.Columns["colBonusUnit"].Width = 50;
                        DgvStockSales.Columns["colUnitPrice"].Width = 60;
                        DgvStockSales.Columns["colAmount"].Width = 70;
                        DgvStockSales.Columns["colDiscount"].Width = 60;
                        DgvStockSales.Columns["colDiscountAmount"].Width = 90;
                        DgvStockSales.Columns["colPromoDisc"].Width = 75;
                        DgvStockSales.Columns["colNetAmount"].Width = 90;
                        DgvStockSales.Columns["colDelete"].Width = 55;
                    }
                }
            }
        }
        ...
        private void FillCustomer(string AccountNo)
        {
            ...
            else
            {
                ...
                List<AccountsEL> list1 = Amanager.GetAccount(AccountNo, Operations.IdCompany, Operations.IdProject);
                if (list1.Count > 0)
                {
                    SEditBox.Text = list1[0].AccountName;
                }
        ...
        private void ClearControls()
        {
            ...
            txtRegularDiscount.Text = string.Empty;
            txtSpecialDiscount.Text = string.Empty;
        ...
        private bool ValidateControls()
        {
            ...
            {
                MessageBox.Show("Please Check Transaction Type...");
                return false;
            }
            //if (VDate.Value == dueDate.Value)
            //{
            //    MessageBox.Show("Invoice Bill Date and Due Date must be different....");
            //    return false;
            //}
            ...
            if (ValidateRows())
            {
                if (ValidateControls())
                {
                    if (ValidateBookPeriod())
                    {
                        ...
                        //if (VoucherDateTime.Value != VDate.Value && VDate.Value > VoucherDateTime.Value)
                        ...
                        oelVoucher.RegularDiscount = Validation.GetSafeDecimal(txtRegularDiscount.Text);
                        oelVoucher.SpecialDiscount = Validation.GetSafeDecimal(txtSpecialDiscount.Text);
                        ...
                        if(txtDescription.Text == string.Empty)
                            oelSaleTransaction.Description = "Sales To : " + SEditBox.Text + " @ " + txtTotalAmount.Text;
                        else
        ...
        private void txtExtraDiscount_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            {
                e.Handled = true;
                decimal ActualAmount = Validation.GetSafeDecimal(txtBillAmount.Text) - (Validation.GetSafeDecimal(txtRegularDiscount.Text) + Validation.GetSafeDecimal(txtSpecialDiscount.Text) + Validation.GetSafeDecimal(txtExtraDiscount.Text));
                decimal Resultant = Validation.GetSafeDecimal(txtFreightAmount.Text);
                txtAmountAfterDiscount.Text = Validation.GetSafeString(ActualAmount);
        ...
        {
            //decimal ActualAmount = Validation.GetSafeDecimal(txtBillAmount.Text) - (Validation.GetSafeDecimal(txtRegularDiscount.Text) + Validation.GetSafeDecimal(txtSpecialDiscount.Text));
            //decimal Resultant = Validation.GetSafeDecimal(txtExtraDiscount.Text);
            //txtAmountAfterDiscount.Text = Validation.GetSafeString((ActualAmount +  Validation.GetSafeDecimal(txtFreightAmount.Text)) - Resultant);
            ...
            if (e.ColumnIndex == 27)
            {
                if (DgvStockSales.Rows[e.RowIndex].Cells["ColIdVoucherDetail"].Value == null)
                {
                    ...
                    for (int i = 0; i < DgvStockSales.Rows.Count - 1; i++)
                    {
                        ...
                        //NormalDiscount += Validation.GetSafeDecimal(DgvStockSales.Rows[i].Cells["colDiscount"].Value);
                        //SpecialDiscount += Validation.GetSafeDecimal(DgvStockSales.Rows[i].Cells["colPromoDisc"].Value);
                        //NetAmount += Validation.GetSafeDecimal(DgvStockSales.Rows[i].Cells["colNetAmount"].Value);

                        BillingAmount += Validation.GetSafeDecimal(DgvStockSales.Rows[i].Cells["colAmount"].Value);
                        string NormalDiscountCellValue = Validation.GetSafeString(DgvStockSales.Rows[i].Cells["colDiscount"].Value);
                        decimal NormalDiscountPercentageValue = 0;
                        if (NormalDiscountCellValue.Contains('%'))
                        {
                            NormalDiscountCellValue = NormalDiscountCellValue.Substring(0, NormalDiscountCellValue.Length - 1);
                            NormalDiscountPercentageValue = ((Validation.GetSafeDecimal(DgvStockSales.Rows[i].Cells["colAmount"].Value) / 100)
                                                                        * Validation.GetSafeDecimal(NormalDiscountCellValue));
                        }
                        else
                        {
                            NormalDiscountPercentageValue = ((Validation.GetSafeDecimal(DgvStockSales.Rows[i].Cells["colAmount"].Value) / 100)
                        ...
                        string SpecialDiscountCellValue = Validation.GetSafeString(DgvStockSales.Rows[i].Cells["colPromoDisc"].Value);
                        decimal SpecialDiscountPercentageValue = 0;
                        if (SpecialDiscountCellValue.Contains('%'))
                        {
                            SpecialDiscountCellValue = SpecialDiscountCellValue.Substring(0, SpecialDiscountCellValue.Length - 1);
                            SpecialDiscountPercentageValue = ((Validation.GetSafeDecimal(DgvStockSales.Rows[i].Cells["colDiscountAmount"].Value) / 100)
                                                                                            * Validation.GetSafeDecimal(SpecialDiscountCellValue));
                        }
                        else
                        {
                            SpecialDiscountPercentageValue = ((Validation.GetSafeDecimal(DgvStockSales.Rows[i].Cells["colDiscountAmount"].Value) / 100)
                        ...
                        NormalDiscount += Validation.GetSafeDecimal(NormalDiscountPercentageValue);
                        SpecialDiscount += Validation.GetSafeDecimal(SpecialDiscountPercentageValue);
                        NetAmount += Validation.GetSafeDecimal(DgvStockSales.Rows[i].Cells["colNetAmount"].Value);
                    }
                    txtBillAmount.Text = Validation.GetSafeString(BillingAmount);
                    txtRegularDiscount.Text = Validation.GetSafeString(NormalDiscount);
                    txtSpecialDiscount.Text = Validation.GetSafeString(SpecialDiscount);
                    txtAmountAfterDiscount.Text = Validation.GetSafeString((BillingAmount - (NormalDiscount + SpecialDiscount + Validation.GetSafeDecimal(txtExtraDiscount.Text))));
                ...
                else
                {
                    if (!chkPosted.Checked)
                    {
                        ...
                        //DgvStockSales.Rows.RemoveAt(e.RowIndex);
        ...
        private void DgvStockSales_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            decimal ConvertedQty = 0, ConvertedQuantityWithOutBonus = 0;
                ...
                if (IdVoucher == 0 || IdVoucher == null)
                {
                    if (ClosingStock > 0)
                    {
                        ...
                        if (Validation.GetSafeDecimal(DimsArray[1]) == 0)
                        {
                            ...
                            ConvertedQuantityWithOutBonus = (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colQty"].Value) * AssignQuantity) + (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colPieces"].Value));
                        }
                        else
                        {
                            ...
                            ConvertedQuantityWithOutBonus = (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colQty"].Value) * Validation.GetSafeDecimal(DimsArray[1]) * Validation.GetSafeDecimal(DimsArray[2])) + Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colBoxes"].Value) * Validation.GetSafeDecimal(DimsArray[2]) + (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colPieces"].Value));
                        }
                        ...
                        if (ClosingStock > ConvertedQty || ClosingStock == ConvertedQty)
                        {
                            /// Allow Input
                            ...
                            {
                                DgvStockSales.Rows[e.RowIndex].Cells["colAmount"].Value = Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colUnitPrice"].Value) * ConvertedQuantityWithOutBonus; //ConvertedQty;
                            }
                            else
                            {
                                DgvStockSales.Rows[e.RowIndex].Cells["colAmount"].Value = Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colUnitPrice"].Value) * ConvertedQuantityWithOutBonus;//ConvertedQty;
                        ...
                        {
                            MessageBox.Show("Less Cartons Availables In Closing Stock.....");
            ...
            else if (DgvStockSales.Columns[e.ColumnIndex].Name == "colBoxes")
            {
                ...
                //{
                    if (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colBoxes"].Value) > 0)
                    {
                        if (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colQty"].Value) > 0)
                        {
                            ...
                            else
                            {
                                ...
                                ConvertedQuantityWithOutBonus = (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colQty"].Value) * Validation.GetSafeDecimal(DimsArray[2])) + (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colPieces"].Value));
                            }
                        }
                        else
                        {
                            ...
                            ConvertedQuantityWithOutBonus = (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colBoxes"].Value) * Validation.GetSafeDecimal(DimsArray[2])) + (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colPieces"].Value));
                        }
                    }
                    else if (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colQty"].Value) > 0)
                    {
                        if (Validation.GetSafeDecimal(DimsArray[1]) > 0)
                        {
                            ConvertedQty = (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colQty"].Value) * Validation.GetSafeDecimal(DimsArray[1]) * Validation.GetSafeDecimal(DimsArray[2])) + (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colBoxes"].Value) * Validation.GetSafeDecimal(DimsArray[2])) + (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colPieces"].Value) + Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colBonusUnit"].Value));
                    ...
                    else if (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colPieces"].Value) > 0)
                    {
                        if (Validation.GetSafeDecimal(DimsArray[1]) > 0)
                        {
                            ConvertedQty = (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colQty"].Value) * Validation.GetSafeDecimal(DimsArray[1]) * Validation.GetSafeDecimal(DimsArray[2])) + (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colBoxes"].Value) * Validation.GetSafeDecimal(DimsArray[2])) + (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colPieces"].Value) + Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colBonusUnit"].Value));
                    ...
                    if (IdVoucher == 0 || IdVoucher == null)
                    {
                        ...
                        {
                            MessageBox.Show("Stock Not Availabled For Sales");
                            DgvStockSales.Rows[e.RowIndex].Cells["colBoxes"].Value = "";
                            return;
                        }
                    }
                    else
                    {
                        ...
                        if (UnpostedQuantity + ClosingStock >= ConvertedQty)
                        {
                            // Take No Action Please....
                        }
                        else
                    ...
                    //if (ClosingStock < ConvertedQty)
                    //{
                    //    MessageBox.Show("Less Stock Available For Sales");
                    //    DgvStockSales.Rows[e.RowIndex].Cells["colBoxes"].Value = "";
                    //    return;
                    //}
                    //else
                    //{
                        ...
                        else
                        {
                            ...
                            if (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colBoxes"].Value) > 0)
                            {
                                if (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colAmount"].Value) == 0)
                                {
                                    DgvStockSales.Rows[e.RowIndex].Cells["colAmount"].Value = Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colUnitPrice"].Value) * ConvertedQuantityWithOutBonus; //ConvertedQty;
                                }
                            }
            ...
            else if (DgvStockSales.Columns[e.ColumnIndex].Name == "colPieces")
            {
                if (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colPieces"].Value) > 0)
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            ConvertedQuantityWithOutBonus = (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colPieces"].Value));
                        }
                    }
                }
                else if (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colBoxes"].Value) > 0)
                {
                    if (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colQty"].Value) > 0)
                    {
                        ConvertedQty = (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colQty"].Value) * Validation.GetSafeDecimal(DimsArray[1]) * Validation.GetSafeDecimal(DimsArray[2])) + (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colBoxes"].Value) * Validation.GetSafeDecimal(DimsArray[2])) + (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colPieces"].Value) + Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colBonusUnit"].Value));
                        //ConvertedQuantityWithOutBonus = ConvertedQty = (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colQty"].Value) * Validation.GetSafeDecimal(DimsArray[1]) * Validation.GetSafeDecimal(DimsArray[2])) + (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colBoxes"].Value) * Validation.GetSafeDecimal(DimsArray[2])) + (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colPieces"].Value));
                        ConvertedQuantityWithOutBonus = (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colQty"].Value) * Validation.GetSafeDecimal(DimsArray[1]) * Validation.GetSafeDecimal(DimsArray[2])) + (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colBoxes"].Value) * Validation.GetSafeDecimal(DimsArray[2])) + (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colPieces"].Value));
                    }
                    else
                    {
                ...
                else if (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colQty"].Value) > 0)
                {
                    ...
                    {
                        ConvertedQty = (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colQty"].Value) * Validation.GetSafeDecimal(DimsArray[0]) * Validation.GetSafeDecimal(DimsArray[2])) + (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colBoxes"].Value) * Validation.GetSafeDecimal(DimsArray[2])) + (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colPieces"].Value) + Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colBonusUnit"].Value));
                        //ConvertedQuantityWithOutBonus = ConvertedQty = (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colQty"].Value) * Validation.GetSafeDecimal(DimsArray[1]) * Validation.GetSafeDecimal(DimsArray[2])) + (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colBoxes"].Value) * Validation.GetSafeDecimal(DimsArray[2])) + (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colPieces"].Value));
                        ConvertedQuantityWithOutBonus = (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colQty"].Value) * Validation.GetSafeDecimal(DimsArray[0]) * Validation.GetSafeDecimal(DimsArray[2])) + (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colBoxes"].Value) * Validation.GetSafeDecimal(DimsArray[2])) + (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colPieces"].Value));
                    }
                }
                ...
                //{
                    if (Validation.GetSafeLong(DgvStockSales.Rows[e.RowIndex].Cells["colAmount"].Value) > 0)
                    {
                        ...
                        {
                            DgvStockSales.Rows[e.RowIndex].Cells["colAmount"].Value = Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colUnitPrice"].Value) * ConvertedQuantityWithOutBonus;
                        }
                    }
                    else
                    {
            ...
            else if (DgvStockSales.Columns[e.ColumnIndex].Name == "colBonusUnit")
            {
                if (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colPieces"].Value) > 0)
                {
                    if (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colBoxes"].Value) > 0)
                    {
                        ...
                        ConvertedQuantityWithOutBonus = (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colQty"].Value) * Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colBoxes"].Value) * Validation.GetSafeDecimal(DimsArray[2])) + (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colPieces"].Value));
                    }
                    else
                    {
                        if (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colQty"].Value) > 0)
                        {
                            ...
                            {
                                ConvertedQty = (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colQty"].Value) * Validation.GetSafeDecimal(DimsArray[1]) * Validation.GetSafeDecimal(DimsArray[2])) + (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colPieces"].Value) + Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colBonusUnit"].Value));
                                ConvertedQuantityWithOutBonus = (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colQty"].Value) * Validation.GetSafeDecimal(DimsArray[1]) * Validation.GetSafeDecimal(DimsArray[2])) + (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colPieces"].Value));
                            }
                            else
                            {
                                ConvertedQty = (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colQty"].Value) * Validation.GetSafeDecimal(DimsArray[2])) + (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colPieces"].Value) + Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colBonusUnit"].Value));
            ...
            else if (DgvStockSales.Columns[e.ColumnIndex].Name == "colUnitPrice")
            {
                ...
                DgvStockSales.Rows[e.RowIndex].Cells["colAmount"].Value = ConvertedQuantityWithOutBonus * Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colUnitPrice"].Value); //ConvertedQty * 
            ...
            else if (DgvStockSales.Columns[e.ColumnIndex].Name == "colPromoDisc")
            {
                if (DgvStockSales.Rows[e.RowIndex].Cells["colPromoDisc"].Value != null)
                {
                    ...
                    if (Validation.GetSafeDecimal(CellValue) > 0)
                    {
                        DgvStockSales.Rows[e.RowIndex].Cells["colNetAmount"].Value = (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colDiscountAmount"].Value) - ((Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colDiscountAmount"].Value) / 100)
                                                                                    * Validation.GetSafeDecimal(CellValue)));
                        DgvStockSales.Rows[e.RowIndex].Cells["colPromoDisc"].Value = CellValue + "%";
                    }
                    else
                    {
                        DgvStockSales.Rows[e.RowIndex].Cells["colNetAmount"].Value = Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colDiscountAmount"].Value);
        ...
        {
            //decimal DiscTotalAmount = 0, AssignQuantity = 0, ConvertedQty = 0, ConvertedQuantityWithOutBonus = 0;
            //string[] DimsArray = new string[3];
            //string Diemension = Validation.GetSafeString(DgvStockSales.Rows[e.RowIndex].Cells["colDims"].Value);
            //if (Diemension != string.Empty)
            //{
            ...
            //    txtRegularDiscount.Text = DiscTotalAmount.ToString();
            //    OldValue = 0;
            //}
            //else if (DgvStockSales.Columns[e.ColumnIndex].Name == "colDiscount")
            //{
        ...
        private void FillVoucher()
        {
            ...
            if (ListSales.Count > 0)
            {
                ...
                txtRegularDiscount.Text = Validation.GetSafeString(ListSales[0].RegularDiscount);
                txtSpecialDiscount.Text = Validation.GetSafeString(ListSales[0].SpecialDiscount);
                ...
                if (Validation.GetSafeDecimal(txtAmountAfterDiscount.Text) == 0)
                {
                    txtAmountAfterDiscount.Text = txtBillAmount.Text;
                }
        ...
        private void FillSales(List<VoucherDetailEL> List)
        {
            ...
            if (List != null && List.Count > 0)
            {
                ...
                for (int i = 0; i < List.Count; i++)
                {
                    ...
                    DgvStockSales.Rows[i].Cells[25].Value = List[i].PromoDiscount.ToString() + "%";
                    DgvStockSales.Rows[i].Cells[26].Value = List[i].NetSaleAmount;
                }
                //txtRegularDiscount.Text = (List.Sum(x => x.Amount) - List.Sum(x => x.DiscountAmount)).ToString();
            }
        }
