    ...
    public partial class frmDistributionSalesReturn : MetroForm
    {
            ...
            if (list.Count > 0)
            {
                ...
                if (objActiveType != null)
                {
                    ...
                    else if (objActiveType.SoftwareType == "Distribution Trading")
                    {
                        ...
                        DgvSales.Columns["colSalesItemName"].Width = 235;
                        ...
                        DgvSales.Columns["colSalesUnitPrice"].Width = 80;
                    ...
                    if (ValidateBookPeriod())
                    {
                        ...
                        if (chkPosted.Checked)
                        {
                            ...
                            for (int k = 0; k < DgvStockSales.Rows.Count - 1; k++)
                            {
                                var IManager = new ItemsBLL();
                                List<ItemsEL> EachItem = IManager.GetItemById(Validation.GetSafeLong(DgvStockSales.Rows[k].Cells["colIdItem"].Value));
                                string[] Diemension = Validation.GetSafeString(DgvStockSales.Rows[k].Cells["colDimension"].Value).Split('*');
                                if (Diemension[1] == "0")
                                {
        ...
        private void FillInvoiceSales(List<VoucherDetailEL> List)
        {
            ...
            if (List != null && List.Count > 0)
            {
                ...
                for (int i = 0; i < List.Count; i++)
                {
                    ...
                    DgvSales.Rows[i].Cells[25].Value = List[i].PromoDiscount;
                    DgvSales.Rows[i].Cells[26].Value = List[i].NetSaleAmount;
        ...
        private void FillVoucherByNumber()
        {
            ...
            if (ListSales.Count > 0)
            {
                ...
                {
                    VoucherDateTime = ListSales[0].EditedDateTime.Value;
                    VDate.Value = ListSales[0].EditedDateTime.Value;
                }
                else
                {
                    //VEditedDateTime.Value = VDate.Value;
        ...
        private void VDate_ValueChanged(object sender, EventArgs e)
        {
            if (DateStatus == 0)
                IsEditedDateChanged = false;
            else
