...
namespace Accounts.UI
{
    public partial class frmStockReturn : MetroForm
    {
            ...
            if (list.Count > 0)
            {
                ...
                if (objActiveType != null)
                {
                    ...
                    else if (objActiveType.SoftwareType == "Distribution Trading")
                    {
                        ...
                        DgvStockReceipt.Columns["colItemName"].Width = 412;
                        DgvStockReceipt.Columns["colpacking"].Width = 100;
                        DgvStockReceipt.Columns["colQty"].Width = 100;
                        DgvStockReceipt.Columns["colUnitPrice"].Width = 100;
                    }
        ...
        private bool ValidateControls()
        {
            ...
            {
                MessageBox.Show("Supplier Account Missing...");
                return false;
            }
            if (rdCredit.Checked)
            {
        ...
        private void DgvStockReceipt_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (DgvStockReceipt.Columns[e.ColumnIndex].Name == "colUnitPrice")
            {
                ...
                //Validation.GetSafeDecimal(DgvStockReceipt.Rows[e.RowIndex].Cells["colUnitPrice"].Value);
                if (QuantityArray[1] == "0")
                {
                    DgvStockReceipt.Rows[e.RowIndex].Cells["colAmount"].Value = Validation.GetSafeDecimal(DgvStockReceipt.Rows[e.RowIndex].Cells["colUnitPrice"].Value) * (Validation.GetSafeDecimal(DgvStockReceipt.Rows[e.RowIndex].Cells["colQty"].Value) * Validation.GetSafeDecimal(QuantityArray[2]));
                }
