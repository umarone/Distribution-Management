...
namespace Accounts.UI
{
    public partial class frmStockReceipt : MetroForm
    {
        ...
        private void frmStockReceipt_Load(object sender, EventArgs e)
        {
            ...
            //}
            this.Text = "Inventory Receipt Note";
        }
            ...
            if (list.Count > 0)
            {
                ...
                if (objActiveType != null)
                {
                    ...
                    if (objActiveType.SoftwareType == "Distribution Trading")
                    {
                        ...
                        DgvStockReceipt.Columns["colItemName"].Width = 260;
                        DgvStockReceipt.Columns["colQty"].Width = 80;
                        DgvStockReceipt.Columns["colDimension"].Width = 80;
                        DgvStockReceipt.Columns["colUnitPrice"].Width = 80;
                    }
        ...
        private string BuildRemarks()
        {
            ...
            for (int i = 0; i < DgvStockReceipt.Rows.Count - 1; i++)
            {
                Remarks += DgvStockReceipt.Rows[i].Cells[3].Value.ToString() + " - "
                           ...
                           + "@" + DgvStockReceipt.Rows[i].Cells[23].Value.ToString() + "~";
            }
            First += Remarks;
            return First;
        }
                ...
                if (ValidateControls())
                {
                    if (ValidateBookPeriod())
                    {
                        ...
                        for (int i = 0; i < DgvStockReceipt.Rows.Count - 1; i++)
                        {
                            ...
                            string Diemension = Validation.GetSafeString(DgvStockReceipt.Rows[i].Cells["colDimension"].Value);
        ...
        void frmfindProducts_ExecuteFindPorudctsEvent(object Sender, ItemsEL oelItems)
        {
            ...
            //if (manager.VerifyAccount(Operations.IdCompany, "Items", oelItems.AccountNo).Count > 0)
            {
                ...
                //    }
                //}
                List<ItemsEL> obj = manager.GetItemById(oelItems.IdItem.Value);
                ...
                if (obj.Count > 0)
                {
                    DgvStockReceipt.CurrentRow.Cells["colDimension"].Value = obj[0].Diemension;
                }
        ...
        {
            var manager = new ItemsBLL();
            string[] QuantityArray = Validation.GetSafeString(DgvStockReceipt.Rows[e.RowIndex].Cells["colDimension"].Value).Split('*');
        ...
        private void FillPurchases(List<VoucherDetailEL> List)
        {
            ...
            if (List != null && List.Count > 0)
            {
                ...
                for (int i = 0; i < List.Count; i++)
                {
                    ...
                    DgvStockReceipt.Rows[i].Cells[20].Value = List[i].Discount;
                    DgvStockReceipt.Rows[i].Cells[21].Value = List[i].DiscountAmount;
                    DgvStockReceipt.Rows[i].Cells[22].Value = List[i].FlatDiscount;
                    DgvStockReceipt.Rows[i].Cells[23].Value = List[i].NetAmount;
        ...
        private void txtBiltyNo_Click(object sender, EventArgs e)
        {

        }

        private void txtBillNo_Click(object sender, EventArgs e)
        {
