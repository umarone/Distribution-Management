...
namespace Accounts.UI
{
    public partial class frmProductFocLedger : MetroForm
    {
        ...
        {
            if (grdProductLedger.Rows.Count > 0)
            {
                DataTable dt = new DataTable();

                //Adding the Columns
                foreach (DataGridViewColumn column in grdProductLedger.Columns)
                {
                    if (column.Visible)
                    {
                ...
                // Add Empty Row....
                dt.Rows.Add();
                for (int i = 0; i < grdProductLedger.Columns.Count; i++)
                {
                ...
                foreach (DataGridViewRow row in grdProductLedger.Rows)
                {
                    dt.Rows.Add();
                    int colindex = 0;
