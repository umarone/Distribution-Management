...
{
    public partial class frmSalesManSalesSummaryPivot : MetroForm
    {
        DataTable dt;
        public frmSalesManSalesSummaryPivot()
        {
            InitializeComponent();
        }
        ...
        private void btnLoad_Click(object sender, EventArgs e)
        {
            ...
            dt = manager.GetAllSalesManSaleSummaryDateWise(Operations.IdProject, Operations.BookNo, dtSummary.Value);
            if (dt.Rows.Count > 0)
            {
        ...
        private void btnExport_Click(object sender, EventArgs e)
        {
            if (grdSummary.Rows.Count > 0)
            {
                ...
                //Adding the Columns
                foreach (DataGridViewColumn column in grdSummary.Columns)
                {
                    if (column.Visible)
                    {
                ...
                // Add Empty Row....
                dt.Rows.Add();
                for (int i = 0; i < grdSummary.Columns.Count; i++)
                {
                ...
                foreach (DataGridViewRow row in grdSummary.Rows)
                {
                    dt.Rows.Add();
                    int colindex = 0;
